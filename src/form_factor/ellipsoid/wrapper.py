"""
Python wrapper for ellipsoid form_factor C extension.
"""
...
_lib_path = find_library("ellipsoid", os.path.dirname(__file__))

if not os.path.exists(_lib_path):
    raise ImportError(
        f"Ellipsoid shared library not found at {_lib_path}. "
...
_lib.Fq.argtypes = [
        ...
        ctypes.c_double,  # radius_polar,
        ctypes.c_double,  # radius_equatorial
    ]
_lib.Fq.restype = None


def compute_form_factor(q, sld, sld_solvent, radius_polar, radius_equatorial, **kwargs):
    """
    ...
    radius_polar : float
        Parameter radius_polar.
    radius_equatorial : float
        Parameter radius_equatorial.
    **kwargs : dict
