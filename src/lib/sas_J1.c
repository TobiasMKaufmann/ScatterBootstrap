...
float cephes_j1f(float xx)
{
    ...
    if( x <= 2.0 ) {
        z = x * x;
        p = (z-Z1) * x * polevl( z, (const double *)JPJ1, 4 );
        return( xx < 0. ? -p : p );
    }
    ...
    p = w * polevl( q, (const double *)MO1J1, 7);
    w = q*q;
    // 2017-05-19 PAK improve accuracy using trig identies
    // original:
    ...
    xn = q * polevl( w, (const double *)PH1J1, 7);
    float cos_xn, sin_xn;
    float cos_x, sin_x;
    SINCOS(xn, (double *)&sin_xn, (double *)&cos_xn);  // about xn and 1
    SINCOS(x, (double *)&sin_x, (double *)&cos_x);
