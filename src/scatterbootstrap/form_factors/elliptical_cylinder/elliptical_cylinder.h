#ifndef ELLIPTICAL_CYLINDER_H
#define ELLIPTICAL_CYLINDER_H

/**
 * Compute the elliptical_cylinder form factor.
 * 
 * @param q Scattering vector magnitude (1/Angstrom)
 * @param F1 Output: Form factor amplitude
 * @param F2 Output: Form factor squared
 * @param ... Model-specific parameters
 */
void compute_elliptical_cylinder_form_factor(double q, double *F1, double *F2, ...);

#endif // ELLIPTICAL_CYLINDER_H
