...
#include <math.h>
#include <stdio.h>
#include "../../lib/gauss76.h"
#include "../../lib/polevl.h"
...
static double _cyl(double vd, double besarg, double siarg) {
    const double sinx_x = sas_sinx_x(siarg);
    const double J1x_x = sas_2J1x_x(besarg);
    return vd * sinx_x * J1x_x;
}

static double form_volume(double radius, double thickness, double length) {
    return M_PI * pow(radius + thickness, 2) * (length + 2.0 * thickness);
}

static double radius_from_excluded_volume(double radius, double thickness, double length) {
...
static double radius_from_volume(double radius, double thickness, double length) {
    const double volume_outer_cyl = form_volume(radius, thickness, length);
    return cbrt(volume_outer_cyl / M_4PI_3);
}

static double radius_from_diagonal(double radius, double thickness, double length) {
...
static double radius_effective(int mode, double radius, double thickness, double length) {
    switch (mode) {
        default:
...
void Fq(double q, double *F1, double *F2, double core_sld, double shell_sld, double solvent_sld, double radius, double thickness, double length) {
    ...
    for (int i = 0; i < GAUSS_N; i++) {
        ...
        SINCOS(theta, (double *)&sin_theta, (double *)&cos_theta);
